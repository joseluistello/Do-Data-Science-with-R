...
```
We can submit this result to Kaggle. and we have got 76.555% accurate! That is not too bad. 
...
R has provided many useful library for claasification, we can make use of them and improve our classifier.
```{r}
...
fancyRpartPlot(model)
```
The decision tree that have been built goes a lot deeper than what we saw last time. Notice that the tree is a binary tree. For test conditions that more than two possible answers have been changed to a binary by auto add a split with them. For example, age are numbers and have 10s of possibilities, our model simple split it by a test dondition `Age >= 8.6`. Conditions have been set for others as well. This conditions are not ideal, they can be changed if you know how to optimize decision tree. For the moment it  looks very promising that resonates with the famous naval law that "women and kids first" is visible in our model. 
