...
```{r material, fig.align = 'center', out.width = "80%", echo=FALSE, fig.cap =""}
...
The protocol is you copy lines indicated from this book "chunk" by "chunk" into your R file and run them. Okay, let us start,
...
- `train` dataset has **891 records** and each record has **12 attributes**. 

Notice that R uses statistics terminology, observation is record in data science term, properties of an observation are attributes of a record^[The same terms have different names in Machine Learning, which we will mention later, but observation is call samples, properties are called variables]. Notice that train has a type of `data.frame`. **Data.frame** is the most used data type in R. Try type `?data.frame` to explore more. We will talk about a dataframe, we usually say dataset. So train is called "train dataset" and test is called "test dataset". 

- `test` dataset has **418 records** and each record has **11 attributes**, which are less than train's in both number of records and attributes. 
...
```{block2,  type='rmdthinking', warn = FALSE}
Thinking:
...
```
So we know the survive rate in the train dataset is about 61%. This is interesting because it reflects the overall survival rate.
...
```{r Survivalrate, fig.cap = "Total count and survive rate of passenger on Pcalss.", fig.align = 'center'}
...
```{r Survivalrateonsex, fig.cap = "Total count and survive rate of passenger on sex.", fig.align = 'center'}
...
Although we have examined the records' numbers in attributes level. We have a good knowledge about the record numbers in the given datasets. It is still necessary to check at the record level. On one hand, if there are some records that have too many missing value for some attributes, for example, despite the records have ids and names, the useful attributes' value are missing.  These records are bed, not useful or invalid records. They should be removed or the missing values should be filled in order a valid analyse can be conducted. 

On other hand, some records have identical values for th emost attributes. These could be considered as duplicates. Depends on the problem to be solved, they could be problematic and need also to be dealt with. 

In our Titanic problem, record level assessment is not an issue since almost of the records are different. This does not mean we should completely ignore this step and not doing any record level checking.   
