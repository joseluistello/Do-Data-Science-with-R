...
# Death and survived value distribution among embarked ports
DeathandsurviveDistribution <- prop.table(SurviveOverEmbarkedTable, 2)
#Convert to data frame
Dataframe.deathandsurviveDistribution <- data.frame(DeathandsurviveDistribution) 
#check their value
#str(Dataframe.deathandsurviveDistribution)

# Take the Death distribution over embarked
Deathdistribution.over.Embarked <- Dataframe.deathandsurviveDistribution$Freq[Dataframe.deathandsurviveDistribution$Var2==0]
# Take the survive distribution over Embarked
Survivedistribution.over.Embarked <- Dataframe.deathandsurviveDistribution$Freq[Dataframe.deathandsurviveDistribution$Var2==1]

# Print out death distribution numbers among Embarked
prop.table(Deathdistribution.over.Embarked)
#Print out Survive distribution numbers among Embarked
prop.table(Survivedistribution.over.Embarked)

M <- c("c", "Q", "S")
# plot Death distribution over embarked port ignore the missing data 
barplot(Deathdistribution.over.Embarked[2:4]*100, xlab =(""),  ylab="Death distribution in percentage %",  names.arg = M, col="steelblue", main="Death distribution", border="black")

# plot Survive Rate over Embarked port ignore the missing data 
barplot(Survivedistribution.over.Embarked[2:4]*100, xlab =("Embarked"),  ylab="Survive distribution in percentage %",  names.arg = M, col="steelblue", main="Survive distribution", border="black" )

## Calculate Death and survived RATE distribution among embarked ports
proptable.deathandsurviveRate <- prop.table(SurviveOverEmbarkedTable, 1)
# Convert deathandsurviveRate over Embarked into data frame
probtable.dataframe.deathandsurviveRate <- data.frame(proptable.deathandsurviveRate) 
#str(probtable.dataframe.deathandsurviveRate)
# Split the Death rate of each embarked port out 
DeathRate.over.Embarked <- probtable.dataframe.deathandsurviveRate$Freq[probtable.dataframe.deathandsurviveRate$Var2==0]
# Plot Death Rate over Embarked ignore the missing data 
barplot(DeathRate.over.Embarked[2:4]*100, xlab =(""),  ylab="Death Rate in percentage %",  names.arg = M, col="steelblue", main="Death Rate Comparision", border="black" )
```
