...
Among many prediction models, *decision tree* is the simplest. It can build many test conditions and split data samples into sub sets, until there is not test conditions to test or there is no  sample left untested^[detailed decision tree and prediction model will be covered in the next chapter].  
...
```{r}
# confirm Age misisng values
summary(data$Age)
# Construct a decision tree with selected attributes and ANOVA method
Agefit <- rpart(Age ~ Survived + Pclass + Sex + SibSp + Parch + Fare + Embarked,
                  data=data[!is.na(data$Age),], 
                  method="anova")
#Fill AGE missing values with prdiction made by decision tree prediction
data$Age[is.na(data$Age)] <- predict(Agefit, data[is.na(data$Age),])
#confir the missing values ahve been filled
summary(data$Age)
```
above three methods all can fill the missing values. Each filled with different values. Depends on the applications you can choose to use any of them. For our prediction problem, I would use the last method. So I will tied data with removal of the two extra age attributes.
...
# data_full$Title <- as.numeric(data_full$Title)
...
# data_full$Deck <- as.numeric(data_full$Deck)
...
# data_full[1:891, ]
summary(data_full[1:891,c(2:3,5:9,12)])
data.pca <- prcomp(data_full[1:891,c(2:3,5:9,12)], center = TRUE,scale. = TRUE)
summary(data.pca)
```
We have obtained 8 principal components, which named as PC1-8. Each of these explains a percentage of the total variation in the dataset. That is to say: PC1 explains 29% of the total variance, which means that nearly one-thirds of the information in the dataset (8 variables) can be encapsulated by just that one Principal Component. PC2 explains 20% of the variance. So, by knowing the position of a sample in relation to just PC1 and PC2, you can get a very accurate view on where it stands in relation to other samples, as just PC1 and PC2 can explain 49% almost half of the variance.
...
We also have other principal components available although they may have less weights in cpamrision with the first two. Each of other components map differently to the original variables. We can also plot these other components, for example PC3 and PC4,
...
PCA and other methods like Factor analysis and SVD (Singular Value Decomposition) are the general dimension deduction methods. User are encouraged to understadn and make use of them. 

## summary

In this chapter, based on the previous chapter on **Data Understanding**, we have demonstrated some basic tasks needed to performed in the step of the data preprocess or data preparation. Those tasks are either resulted from the initial data quality assessment like discover of the missing values or demanded by the next step of data analyses like correlation analyses to order the potentail predictors based on the prediction power. Attributes re-engineering is the task to make the maximum use of information contained in the give dataset or to transform give attribute in most appropriate form ro types. The ultimate goal is to make datasets ready for analyzing.  
